    ...
    while False not in [x.alive for x in armies.values()]:
        ...
        for army in list(armies.values())[::-1]:
            ...
            for combination in combinations:
                dmg, ta, tgk = combination[0].find_target(combination[1], armies, exclude_defend)
                if ta is None:
                    continue
                target = [combination[0].groups[combination[1]].initiative, combination[0], combination[1], ta, tgk]
                targets.append(target)
                exclude_defend.append(ta.groups[tgk])

        targets.sort(key=lambda row: -row[0])

        for target in targets:
            try:
                target[3].groups[target[4]].defend(target[1], target[2], target[4])
            except KeyError:
                if not DEBUG:
                    print(f'Deleted group {target[2]} from {target[1].name} cannot do anything.')

            if target[3].groups[target[4]].count == 0:
                del target[3].groups[target[4]]

                if not DEBUG:
                    print(f'Deleted group {target[4]} from {target[3].name}.')
